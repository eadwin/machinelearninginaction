...
def file2matrix(filename):
    ...
    classLabeVector = []                    # prepare labels return
    index = 0
    for line in arrayOfLines:
        ...
        if(listFromLine[-1].isdigit()):
            ...
            classLabeVector.append(love_dictionary.get(listFromLine[-1]))
        index += 1
...
def datingClassTest(filename):
    ...
    for i in range(numTestVecs):
        ...
        if (classifierResult != datingLabels[i]):
            print "index %d: the classifier came back with: %d, the real answer is: %d" % \
...
def classifyPerson():
    ...
    percentTats = float(raw_input(\
        "percentage pf time spent playing video games?\n"))
...
def handwritingClassTest():
    hwLabels = []
    trainingFileList = listdir('trainingDigits')  # load thr training set
    m = len(trainingFileList)       # number of the samples
    ...
    errorCount = 0.0                # count the error classification
